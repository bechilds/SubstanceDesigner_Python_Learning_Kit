...
> 当前开发工具版本：v0.24.1（`__init__.py/TOOL_VERSION`）；随插件 v0.25.0 升级。此版本号不代表 MG 已发布版本。
...
flowchart TD
    A[打开当前 Graph] --> B[刷新静态复杂度与文件健康分析]
    B --> C[查看热点与结构警告]
    C --> D{输入与目标确认无误?}
    D -- 否 --> B
    D -- 是 --> E[定位节点 或选择保存报告 确认发布]
    E --> F[查看报告文件或 SBSAR 与 SD 日志]
</whiteboard>

- 从 Published Output 反向遍历，只统计参与最终输出的节点。
- 同时显示当前 Switch 路径、逐个 Switch 选择最高成本输入的 Potential Maximum，以及全部分支总量。
...
- 将节点/Ghost、依赖和 Resource 丢失独立评为文件健康 Error，非标准本地路径评为 Warning，不再混入复杂度分；Designer 安装自带的 `resources/packages` 和 `D:\LG_SDNodes` 视为可信目录。
...
- 支持设置默认报告目录；路径通过插件专属 `QSettings` 缓存，窗口再次打开时自动恢复。
- 支持用 Windows 资源管理器打开当前报告目录，并把报告直接保存为 `<SBS名>_complexity_report.png`，不包含路径栏和底部操作按钮。
...
| `_reachable_nodes()` | 分别计算 Current、Potential Maximum 与全部分支可达节点 |
...
- 文件检查保留原有内部风险权重用于警告排序，但不再加入 Current 与 Potential 复杂度；缺失内容为 Error，可信目录外路径为 Warning。
- 临时等级阈值为 Low `<= 150`、Medium `<= 400`、High `<= 800`、Very High `> 800`，后续应继续使用团队真实 Graph 样本校准。
...
- 同一 SBS 再次保存报告时会覆盖默认目录中的同名 PNG；需要保留历史版本时请先重命名旧报告。
...
- 2026-09-04 · v0.24.1 · 接入统一窗口生命周期 · 本工具入口与错误处理 · 随插件 v0.25.0 升级需重启 SD
